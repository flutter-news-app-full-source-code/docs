...
These variables are required only if you intend to use the push notification feature. The server will start without them, but the corresponding push notification clients will be disabled.

<Aside type="tip">
  For a detailed walkthrough on how to obtain these keys, please follow the [**Configure Push Notifications Guide**](/docs/api-server/guides/configure-push-notifications).
</Aside>
...
The client email from your Firebase service account key.

#### `FIREBASE_PRIVATE_KEY`
The private key from your Firebase service account key.

#### `ONESIGNAL_APP_ID`
The App ID for your OneSignal application.

#### `ONESIGNAL_REST_API_KEY`
The REST API Key for your OneSignal application.
